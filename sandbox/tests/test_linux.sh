#!/bin/bash
# Integration tests for sandbox (Linux)
# Usage: test_linux.sh <path-to-sandbox>

set -u

SANDBOX="$1"
if [ -z "$SANDBOX" ]; then
    echo "Usage: test_linux.sh <path-to-sandbox>"
    exit 2
fi
...
echo "=== Test 6: --shell bash - write outside workspace (should fail) ==="
"$SANDBOX" --shell bash -c "echo hello > $OUTSIDE/bash_test.txt" --workspace "$WS"
rc=$?
if [ $rc -ne 0 ]; then
    echo "[PASS] bash shell write outside workspace denied"
else
    echo "[FAIL] bash shell write outside workspace - should have been denied"
    EXIT_CODE=1
fi
